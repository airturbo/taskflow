/**
 * Lightweight Chinese lunar calendar converter
 * Covers 1900–2100, zero dependencies
 */
...
function solarToLunar(year: number, month: number, day: number) {
  ...
  return {
    ...
    lunarDay,
    isLeap,
    monthName: `${isLeap ? '闰' : ''}${MONTH_NAMES[lunarMonth - 1]}月`,
    dayName: DAY_NAMES[lunarDay - 1] ?? `${lunarDay}`,
    dateStr:
      lunarDay === 1
        ? `${isLeap ? '闰' : ''}${MONTH_NAMES[lunarMonth - 1]}月`
        : DAY_NAMES[lunarDay - 1] ?? `${lunarDay}`,
  }
}

function getSolarTermDay(year: number, termIndex: number) {
  const utcTime = Date.UTC(1900, 0, 6, 2, 5)
    + 31556925974.7 * (year - 1900)
    + SOLAR_TERM_INFO[termIndex] * 60000
...
function getSolarTerm(year: number, month: number, day: number): string | null {
  ...
  if (day === getSolarTermDay(year, firstIndex)) return SOLAR_TERMS[firstIndex]
  if (day === getSolarTermDay(year, firstIndex + 1)) return SOLAR_TERMS[firstIndex + 1]
  return null
}

export function getLunarDate(dateKey: string) {
  ...
  return {
    ...lunar,
    solarTerm,
    display: solarTerm || lunar.dateStr,
  }
}
