...
function WorkspaceApp({ initialState }: { initialState: PersistedState }) {
  ...
  const commitEdit = () => {
    ...
    if (editingTarget.type === 'folder') renameFolder(editingTarget.id, editingTarget.value)
    else renameList(editingTarget.id, editingTarget.value)
  ...
  const navigationContent = (
    <>
      ...
      <SidebarSection title="系统视图">
        <NavButton icon="◎" label="全部" count={countsBySelection['system:all']} active={activeSelection === 'system:all'} onClick={() => setActiveSelection('system:all')} />
        <NavButton
          icon="☀"
          label="今日"
          count={countsBySelection['system:today']}
          active={activeSelection === 'system:today'}
          onClick={() => setActiveSelection('system:today')}
          modeSwitcher={{
            value: selectionTimeModes?.today ?? 'planned',
            onChange: (mode) => {
              setActiveSelection('system:today')
              updateSelectionTimeMode('today', mode)
            },
          }}
        />
        <NavButton
          ...
          onClick={() => setActiveSelection('system:upcoming')}
          modeSwitcher={{
            value: selectionTimeModes?.upcoming ?? 'planned',
            onChange: (mode) => {
              setActiveSelection('system:upcoming')
              updateSelectionTimeMode('upcoming', mode)
            },
          }}
        />
        <NavButton icon="📥" label="收件箱" count={countsBySelection['system:inbox']} active={activeSelection === 'system:inbox'} onClick={() => setActiveSelection('system:inbox')} />
        <NavButton icon="✓" label="已完成" count={countsBySelection['system:completed']} active={activeSelection === 'system:completed'} onClick={() => setActiveSelection('system:completed')} />
        <NavButton icon="🗑" label="回收站" count={countsBySelection['system:trash']} active={activeSelection === 'system:trash'} onClick={() => setActiveSelection('system:trash')} />
      </SidebarSection>
      ...
      >
              ...
              >
                <span
                  ...
                  onClick={e => {
                    ...
                    updateFolderColor(folder.id, next)
                  }}
                />
                ...
                <div className="folder-title__actions">
                  <button
                    ...
                    onClick={async (e) => {
                      e.stopPropagation()
                      const name = await mobilePrompt('清单名称')
                      if (name) createList(name, folder.id)
                    }}
                  >＋</button>
                  <button
                    className="sidebar-action-btn"
              ...
              {lists
                ...
                .map((list) => (
                  <FolderListItem
                    ...
                    onClick={() => setActiveSelection(`list:${list.id}`)}
        ...
        <div
          ...
          role="button"
          tabIndex={0}
          onClick={() => setActiveSelection('list:inbox')}
          onKeyDown={e => handleCardKeyboardActivation(e, () => setActiveSelection('list:inbox'))}
        >
        ...
        {ctxMenu && (
          ...
          >
            {ctxMenu.type === 'folder' ? (
              <>
                <button onClick={async () => { const name = await mobilePrompt('清单名称'); if (name) createList(name, ctxMenu.id); setCtxMenu(null) }}>＋ 新建清单到此文件夹</button>
                ...
                <button className="danger" onClick={async () => {
                  ...
                  if (f && await mobileConfirm(`删除文件夹「${f.name}」？清单会保留。`)) deleteFolder(ctxMenu.id)
                  setCtxMenu(null)
                }}>✕ 删除文件夹</button>
              </>
            ) : (
              <>
                ...
                <button onClick={async () => {
                  ...
                  updateListFolder(ctxMenu.id, folder?.id ?? null)
                  setCtxMenu(null)
                }}>→ 移动到文件夹</button>
                <div className="ctx-menu__divider" />
                <button className="danger" onClick={async () => {
                  const l = lists.find(x => x.id === ctxMenu.id)
                  if (l && await mobileConfirm(`删除清单「${l.name}」？任务移到收件箱。`)) deleteList(ctxMenu.id)
                  setCtxMenu(null)
                }}>✕ 删除清单</button>
              </>
            )}
      ...
      <SidebarSection
        title="标签筛选"
        actions={
          <button className="ghost-button small" onClick={() => setTagManagerOpen(true)}>
            管理
          </button>
        }
      >
      ...
      <SidebarSection title="智能清单">
        {filters.map((filter) => (
          <NavButton
            ...
            onClick={() => setActiveSelection(`filter:${filter.id}`)}
          />
        ))}
      </SidebarSection>

      <SidebarSection title="洞察">
        <NavButton icon="📊" label="统计" active={activeSelection === 'tool:stats'} onClick={() => setActiveSelection('tool:stats')} />
      </SidebarSection>
