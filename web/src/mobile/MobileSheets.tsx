    ...
    <div className="task-sheet-layer" role="dialog" aria-modal="true" aria-label="快速创建">
      ...
      <div className="task-sheet panel mobile-quick-create-sheet">
        ...
        <div className="mobile-quick-create-body">
          ...
          <div className="mobile-quick-create-input-row">
            <input
              ref={inputRef}
            ...
            {voiceSupported && (
              <button
                type="button"
                className={`voice-input-btn ${isListening ? 'is-listening' : ''}`}
          ...
          {/* 快捷日期选择（计划完成时间） */}
          <div className="mobile-quick-create-section">
            <p className="mobile-quick-create-section-label">计划完成</p>
            <div className="mobile-quick-create-date-chips">
              {quickDates.map(opt => (
                <button
                  key={opt.label}
                  className={`mobile-quick-create-date-chip ${selectedDueAt === opt.value ? 'is-active' : ''} ${opt.value === null && selectedDueAt === null ? 'is-active' : ''}`}
                  onClick={() => setSelectedDueAt(opt.value)}
                >{opt.label}</button>
              ))}
              {/* 自定义时间按钮 */}
              <label className={`mobile-quick-create-date-chip mobile-quick-create-date-custom ${selectedDueAt && !quickDates.some(o => o.value === selectedDueAt) ? 'is-active' : ''}`}>
          ...
          {/* 优先级选择 */}
          <div className="mobile-quick-create-row">
            <label>优先级</label>
            <div className="mobile-quick-create-priority-row">
              {priorities.map(p => (
                <button
                  key={p.value}
                  className={`mobile-quick-create-priority-btn p-${p.value} ${selectedPriority === p.value ? 'is-active' : ''}`}
          ...
          {tags.length > 0 && (
            <div className="mobile-quick-create-row">
              <label>标签</label>
              <div className="mobile-quick-create-tags">
                {tags.map(tag => (
                  <button
                    key={tag.id}
                    className={`mobile-quick-create-tag-chip ${selectedTagIds.includes(tag.id) ? 'is-active' : ''}`}
          ...
          {/* 高级选项（开始时间 + DDL） */}
          <button
            className="mobile-quick-create-advanced-toggle"
          ...
          {showAdvanced && (
            <div className="mobile-quick-create-advanced">
              <div className="mobile-quick-create-row mobile-quick-create-time-row">
                <label>开始时间</label>
                <input
                  type="datetime-local"
                  value={selectedStartAt ?? ''}
                  onChange={e => setSelectedStartAt(e.target.value || null)}
                  className="mobile-quick-create-time-input"
                />
              </div>
              <div className="mobile-quick-create-row mobile-quick-create-time-row">
                <label>最终期限</label>
...
export function MobileConfirmSheet({
  ...
  return (
    <div className="task-sheet-layer" role="dialog" aria-modal="true" aria-label="确认">
      ...
      <div className="task-sheet panel mobile-confirm-sheet">
...
export function MobilePromptSheet({
  ...
  return (
    <div className="task-sheet-layer" role="dialog" aria-modal="true" aria-label="输入">
      <button className="task-sheet-backdrop" onClick={onCancel} aria-label="取消" />
      <div className="task-sheet panel mobile-prompt-sheet">
        <div className="task-sheet__handle-area"><div className="task-sheet__handle" /></div>
        <div className="mobile-prompt-body">
          <p className="mobile-prompt-message">{message}</p>
          <input
            ref={inputRef}
...
export function MobileTagManagerSheet({
  ...
  return (
    <div className="task-sheet-layer" role="dialog" aria-modal="true" aria-label="标签管理">
      <button className="task-sheet-backdrop" onClick={onClose} aria-label="关闭" />
      <div className="task-sheet panel mobile-tag-manager-sheet">
        <div className="task-sheet__handle-area">
          <div className="task-sheet__handle" />
        </div>

        {/* 标题栏 */}
        <div className="mobile-tag-manager__header">
          <h3>标签管理</h3>
          <span className="mobile-tag-manager__count">{tags.length} 个标签</span>
          <button className="mobile-tag-manager__close" onClick={onClose} aria-label="关闭">✕</button>
        </div>

        {/* 新建标签 */}
        <div className="mobile-tag-manager__create">
          <div className="mobile-tag-manager__create-row">
            {/* 颜色圆点 — 点击弹出 color swatches */}
            <button
              className="mobile-tag-manager__color-dot"
            ...
            <input
              ref={inputRef}
              className="mobile-tag-manager__create-input"
              placeholder="新标签名称…"
              value={newName}
            ...
            />
            <button
              className="mobile-tag-manager__create-btn"
              disabled={!newName.trim()}
              onClick={handleCreate}
            >添加</button>
          </div>
          {/* 颜色 swatches */}
          <div className="mobile-tag-manager__swatches">
            {TAG_COLOR_PRESETS.map(c => (
              <button
                key={c}
                className={`mobile-tag-swatch ${c === newColor ? 'is-active' : ''}`}
                style={{ background: c }}
                aria-label={`颜色 ${c}`}
          ...
          {error && <p className="mobile-tag-manager__error">{error}</p>}
        </div>

        {/* 标签列表 */}
        <div className="mobile-tag-manager__list">
          {tags.length === 0 && (
            <p className="mobile-tag-manager__empty">还没有标签，快来创建第一个吧！</p>
          )}
          {tags.map(tag => {
            const locked = isSystemTagId(tag.id, Object.values(SPECIAL_TAG_IDS))
            const isEditing = editingId === tag.id
            return (
              <div key={tag.id} className={`mobile-tag-item ${isEditing ? 'is-editing' : ''}`}>
                {isEditing ? (
                  /* 编辑模式 */
                  <div className="mobile-tag-item__edit-row">
                    <button
                      className="mobile-tag-manager__color-dot"
                    ...
                    <input
                      ...
                      style={{ fontSize: 16 }}
                    />
                    <button className="mobile-tag-item__save-btn" onClick={saveEdit}>保存</button>
                    <button className="mobile-tag-item__cancel-btn" onClick={() => setEditingId(null)}>✕</button>
                  </div>
                ) : (
                  /* 查看模式 */
                  <div className="mobile-tag-item__view-row">
                    <span className="mobile-tag-item__dot" style={{ background: tag.color }} />
                    <span className="mobile-tag-item__name">#{tag.name}</span>
                    {locked && <span className="mobile-tag-item__lock-badge">系统</span>}
                    <div className="mobile-tag-item__actions">
                      {!locked && (
                        <>
                          <button className="mobile-tag-item__edit-btn" onClick={() => startEdit(tag)} aria-label="编辑">
                          ...
                          <button className="mobile-tag-item__delete-btn" onClick={() => handleDelete(tag.id)} aria-label="删除">
